...
static void settings_defaults(void) {
    ...
    settings.macro_enabled = 0u;
    settings.macro_count = 0u;
...
static void macro_start(uint32_t now_ms) {
    if (maintenance_mode) {
        cdc_write_line("ERR MACRO MAINTENANCE");
        return;
    }
    if (!settings.macro_enabled || settings.macro_count == 0u) {
        cdc_write_line("ERR MACRO NOT CONFIGURED");
        return;
    }
    macro_running = true;
    macro_index = 0u;
    ...
    cdc_write_line("EVENT MACRO START");
}
...
static void macro_task(uint32_t now_ms) {
    ...
    if (macro_index >= settings.macro_count) {
        macro_stop("EVENT MACRO DONE");
        return;
    }

    const macro_step_t *step = &settings.macro_steps[macro_index];
    macro_keycode = step->keycode;
    macro_key_release_ms = now_ms + step->hold_ms;
    macro_next_step_ms = now_ms + step->hold_ms + step->wait_ms;
    ++macro_index;
    ...
    snprintf(
        line,
        sizeof(line),
        "EVENT MACRO STEP=%u TOTAL=%u",
        (unsigned)macro_index,
        (unsigned)settings.macro_count
    );
    cdc_write_line(line);
}
...
static void handle_button_events(uint32_t now_ms) {
    const bool both_start =
        buttons[BTN_P1_START].stable && buttons[BTN_P2_START].stable;
    ...
    if (!buttons[BTN_P1_START].stable && buttons[BTN_P1_START].previous) {
        if (!start_pair_seen && !maintenance_mode && credits > 0u) {
            --credits;
...
static void scan_buttons(uint32_t now_ms) {
    ...
    const bool relay1_active =
        !maintenance_mode && relay_awake && buttons[BTN_P1_TRIGGER].stable;
    const bool relay2_active =
        !maintenance_mode && relay_awake && buttons[BTN_P2_TRIGGER].stable;
...
static void send_status(void) {
    ...
    snprintf(
        line,
        sizeof(line),
        "STATUS C=%u S1=%u T1=%u B1=%u S2=%u T2=%u B2=%u "
        ...
        (!maintenance_mode && relay_awake && buttons[BTN_P1_TRIGGER].stable) ? 1u : 0u,
        (!maintenance_mode && relay_awake && buttons[BTN_P2_TRIGGER].stable) ? 1u : 0u,
        credits,
...
static void send_config(void) {
    ...
    snprintf(
        line,
        sizeof(line),
        "CONFIG RELAYLOW=%u IDLE=%u KEYPULSE=%u MACROEN=%u MACROCOUNT=%u",
        settings.relay_active_low ? 1u : 0u,
        (unsigned)settings.inactivity_s,
        (unsigned)settings.key_pulse_ms,
        settings.macro_enabled ? 1u : 0u,
        (unsigned)settings.macro_count
    );
    cdc_write_line(line);
}
...
static void process_command(const char *command, uint32_t now_ms) {
    ...
    if (strcmp(command, "PING") == 0) {
        cdc_write_line("PONG TG_CONTROLLER_PRO V4.0.0");
    } else if (strcmp(command, "INFO") == 0) {
        cdc_write_line("INFO NAME=TG_CONTROLLER VERSION=4.0.0 BOARD=RP2040");
        ...
        settings.macro_count = 0u;
        settings.macro_enabled = 0u;
        memset(settings.macro_steps, 0, sizeof(settings.macro_steps));
        macro_stop(NULL);
        cdc_write_line("OK MACRO CLEAR");
    } else if (sscanf(command, "MACRO ADD %u %u %u", &a, &b, &c) == 3 &&
               a <= 255u && b <= 5000u && c <= 60000u &&
               settings.macro_count < MACRO_MAX_STEPS) {
        macro_step_t *step = &settings.macro_steps[settings.macro_count++];
        step->keycode = (uint8_t)a;
        step->hold_ms = (uint16_t)b;
        step->wait_ms = (uint32_t)c;
        cdc_write_line("OK MACRO ADD");
    } else if (sscanf(command, "MACRO ENABLE %u", &a) == 1) {
        settings.macro_enabled = a ? 1u : 0u;
        cdc_write_line("OK MACRO ENABLE");
